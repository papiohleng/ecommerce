...
function runSetup() {
  ...
  // Try container-bound first, then openById
  var ss;
  try {
    ss = SpreadsheetApp.getActiveSpreadsheet();
    if (!ss) throw new Error('Not container-bound');
    Logger.log('Using container-bound spreadsheet');
  } catch(e) {
    Logger.log('Not container-bound, using openById');
...
function testSetup() {
  ...
  try {
    ...
    ss = SpreadsheetApp.openById('1NnsMcz3r6yGfzkXhI3SyF5RwMfKTR2USrxt89Qn5gfI');
  }
